...
//! первый вызов next() запускает генератор, в последующие вызова мы можем передавать данные внутрь генератора, next(data), тем самым если в теле функции let value = yield, то это переменой присвоиться переданое значение. Таким образом мы можем обмениться данными с функцией по мере ее выполнения
...
// TODO Регулярные выражения (RegExp)
//? Есть два способа создать регулярное выражение, 1 - /regexp/, но тут нельзя вставлять переменные, 2 - new RegExp('${name}', 'флаги'), можно вставить переменные
//? Флаги регулярок: i - делает поиск нерегистрозависимым; g - ищет все совпадения, а не только первое; s - с включенным флагом '.' будет соответствовать в том числе и символу перевода строки '\n'; u - включает корректную обработку сурогатных пар, так же нужен для поиска по Юникоду; m - при ключенном флаге якорь '^' ищет не только начало текста, но и начало каждой строки, флаг '$' аналогично ищет не только конец текста но и конец каждой строки. y - позволяет, используя regexp.exec(str), искать совпадения на конкретно заданной позиции и больше не где.
//? Методы: - str.match(regexp) - возвращает массив совпадений (если есть g) или массив с данными с первым элементов равным совпадению, если совпадений нет null. str.replace(regexp, replacement) - заменяет в str совпадения с regexp на replacement. - regexp.test(str) - возвращает boolean в зависимости от того, найдено совпадение или нет. - matchAll возвращает перебираемый объект, в случае влага g каждый элемент это массив со скобочными группами. - regexp.exec(str) - ищет совпадения по заданной позиции, без флагов g и y работает аналогично .match. У regexp есть свойство lastIndex (при создании = 0, можно изменять вручную), exec ищет совпадения по заданному lastIndex и когда находит обновляет lastIndex на позицию следующую за совпадением. С флагом y exec ищет ровно на указанной позиции (lastIndex) ни больше ни меньше.
//? Символьные классы (\class) - соответствуют специальным символам.
//? \d - цифра 0-9; \s - пробел, табуляция (\t), перевод строки, \v \f \r; \w - буква (латиница), цифра или '_'; 
//? обратные сивольные классы, \D - все кроме цифр, \S - все кроме пробелов, \W - все кроме букв(латиницы), цифр или '_' (русские буквы входят в этот класс)
//? Знак '.' - это любой символ кроме новой скроки (\n), флаг s включает поиск новых строк
//? Юникодные свойства (\p{псевдоним}) - поиск символов по свойству Юникода, например псевдоним L(Letter) - буква или N(Number) - цифра
//? Якори '^' - совпадение с началом текста, '$' - совпадение с концом текста. Например /^my/.test('my') - ищет совпадает ли начало строки с 'my'. Так же использования вместе, позволяет захватить весь текс, всю данную строку ^...$
//? Граница слова (\b) - проверка на то, что за /b символ является границей слова (отличные от \w). 
//? Для поиска конкретного "спецального" символа (? : * и т.д) нужно экранировать их, поставив перед ним \. В случае new RegExp экранирование нужно дублировать \\ т.к строки съедают одиночные \
//? Диапазон значений /[]/, например [a-z] диапазон латинский букв от а-z или [\d\s] либо цифра либо пробел. Диапазон отрицания /[^0-9]/ - все кроме цифр от 0 до 9. '-' если он между символами имеет специальное значение.
//? Квантификаторы - указывают на кол-во символов, символьных классов, диапозонов и т.д. Синтаксис \d{a,b} - означает число от a до b раз. Пример \d{3,6} - число с кол-во цифр от 3 до 6 или \d{2,}(второй параметр можно не указывать, тогда будет считать что кол-во от 2 и более) - число с кол-вом цифр от 2. Сокращения + - {1,}, * - {0,}, ? - {0,1}.  
//? Ленивые квантификаторы, {}?, *? и т.д, ищут минимальное кол-во символов проверяя каждый новый подробнее  https://learn.javascript.ru/regexp-greedy-and-laz     
//? Скобочные группы - () объединяют символы в группы для поиска. Например: /([a-z]\d*)+/g, все что в скобках будет длинной от 1 и более раза. Так же в match без флага g каждая скобочная группа будет возвращаться под соотвествующим индексом в массиве результата match. 
//? Скобочным группам можно давать имена добавлением после открыти скобки ?<name>, Пример: /(?<year>\d){4}/, именнованые группы сохраняются в result.groups. Что бы исключить скобочную группу из результата, в ее начало нужно добавить :?, например /(:?max)?/
//? К скобочным группам можно обращаться внутри регулярных выражений, \n, где n - Номер скобочной группы, для именованных скобочных групп \k<name>, обращаясь к скобочной группе движок понимает что нужно найти то же самое 
//? Альтернация(ИЛИ) - синтаксис "|" то есть или левая сторона или правая от |  
//? Опережающая проверка X(?=Y) - найти X при условии что за ним следует Y. Негативная опережающая проверка X(?!Y) - найти X за которым не следует Y. Ретроспективная проверка (?<=Y)X - найти X который следует за Y, (?<!Y)X - найти X, который не следует за Y
//? Расширеная замена $<name/n> - вставляет содержимое скобочной группы по номеру или имени, &$ - совпадение, $` - str до совпадения, $' - str после совпадения, $$ - символ $
// --------------------------------------------------------------------------------------------------------
...
//* асинхронный код всегда выполняется после синхронного (SetTimeout, Promise.then.catch будут выполнены после синхронного кода)
...
//? если разные источники, только изменения location и доступ к iframe.contentWindow
//? window.frames - коллекция фреймов в окне (работает как иерархия)
...
//! ArrayBuffer(n) - класс для создания непрерывной области в памяти длинной n байт (в 1 байте 8 бит)
//! Для использования ArrayBuffer нужно каждый байт как-то представить, для этого используются ТИПИЗИРОВАННЫЕ МАССИВЫ (TypedArray)
//! Например Uint8Array(new ArrayBaffer(8)) - создаст типизированный массив, в котором каждый элемент имеет 8 байт
//! Uint8Array()
//* Виды TypedArray: 
//! Uint8Array, Uint16Array, Uint32Array - целые беззнаковые числа по 8, 16, 32 байта
//! Uint8ClampedArray - 8 битовые целые числа обрезаемые по верхней и нижней границе при присвоении. В него записываются 255 для чисел больше 255 и 0 для отрицательных
//! Int8Array, Int16Array, Int32Array - целые числа со знаком (могут быть отрицательными)
//! Float32Array, Float64Array - 32 и 64 битовые числа со знакоом и плавающей точкой 
//! При создании типизированного массива через любой TypedArray ArrayBuffer можно не использовать, он передается автоматическм 
//! new Int8Array(object, length, TypedArray или ничего) - при таком создании ArrafyBuffer использовать не нужно
//! .buffer - ссылка на объект ArrayBuffer, byteLength - размер ArrayBuffer в байтах
//! Если в TypedArray записывается число больше допустимного то обрезаются биты, самые правые будут записаны, например 256 в 2-ой форме 100000000, при записи в Uint8Array запишутся биты 0000000 обрезав 1 и будет в результате число 0
//! arr.set(fromArr, [offset]) - копирует значения из fromArr в arr с позиции offset
//! arr.subarray([begin, end]) - создает новое представление того же типа для данных с begin до end (удобно использовать для операций над данными)
//* DataViev - позволяет обращаться к данным на любой позиции и в любом формате
//! DataViev(buffer, [byteOffset] - нач. позиция в buffer, [byteLength] - длинна в байтах из buffer) 
//! получить данные .getUint8(index (позиция))[Uint32,Int8 и т.д] 
//? Работа со строками
//! Для декодирования бинарных данных в текст new TextDecoder([label] - тип кодировки (utf-8 и т.д), [options])
//! options = {fatal: boolean - генерация ошибок при невалидных символах , ignoreBOM: boolean - игнор BOM} 
//! .decode([input] - бинарный буффер, [options - {stream: boolean - для декодирования потоков данных}]) 
//! Для кодирования текста new TextEncode() - только utf-8
//! .encode(str) - return Uint8Array с закодированой строкой
//! .encodeInto(str, destination) - кодирует строку помещая ее в destination который должен быть Uint8Array   
//? Blob 
//! Blob - это бинарные данные с типом (type в конструкторе) 
//! new Blob(blobParts - [массив значений Blob, TypedArray, String], options)
//! options {type: utf-8 - тип объекта (в запросах Content-Type), endings - изменяет окончания строк в зависимости от ОС}
//! .slice(start, end, type - тип создаваемого среза)
//! Blob может использоваться как url для показа содержимого, создать ссылу на Blob - URL.createObjectUrl(blob), удалить ссылку URL.revokeObjectUrl(url) (надо т.к Blob будет в памяти пока на него есть ссылка)
//? File/FileReader
//! new File(fileParts - [массив значений Blob, TypedArray, String], filename, options)
//! options {lastModifield} - дата последнего изменения в формате timestamp    
//! Так же объект типа File можно получить из input type=file
//! new FileReader() - читает данные из Blob, File и т.д
//! .readAsArrayBuffer(blob) - читает данные как ArrayBuffer
//! .readAsText(blob, [encoding: utf-8]) - читает данные как строку
//! .readAsDataUrl(blob) - читает данные как base64 кодированый URL (аналог URL.createObjectUrl/revokeObjectUrl)
//! .abort() - отменяет операцию
//! reader генерирует события: loadstart - чтение начало, progress - чтение идет, abort - abort(), error, loaded - чтение завершено
//! .result - результат, . error - ошибка
//! Для WebWorker есть FileReaderSync - синхронное чтение не генерирующее событий 
//TODO Бинарыне данныйе и файлы
